...
def cortar_imagem(imagem):
    ...
    limites_horizontais = np.array([[imagem.size,imagem.size],
                            [0.0, 0.0]], dtype=np.float64)
    limites_verticais = np.array([[imagem.size,imagem.size],
                                    [0.0, 0.0]], dtype=np.float64)
...
def main(nome_arquivo):
    ...
    verdadeiras = ler_csv('./dataset/corretas.csv', nome_arquivo)
